...
function Resizer({ className, ...props }: Props<HTMLDivElement>) {
  ...
  const resizeWindow: (args: any) => any = useCallback((e: JSX.TargetedMouseEvent<HTMLDivElement>) => {
    const size = {
      width: Math.min(windowSize.max.width, Math.max(windowSize.minimized.width, Math.floor(e.clientX + 5))),
      height: Math.min(windowSize.max.height, Math.max(windowSize.minimized.height, Math.floor(e.clientY + 5)))
    };
