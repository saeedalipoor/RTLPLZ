...
export function getCorrectTextColor(hex: string) {
  const threshold: number = 160; /* about half of 256. Lower threshold equals more dark text on dark background  */

  const hRed: number = hexToR(hex);
  const hGreen: number = hexToG(hex);
  const hBlue: number = hexToB(hex);


  function hexToR(h: string): number { return parseInt((cutHex(h)).substring(0, 2), 16) }
  function hexToG(h: string): number { return parseInt((cutHex(h)).substring(2, 4), 16) }
  function hexToB(h: string): number { return parseInt((cutHex(h)).substring(4, 6), 16) }
  function cutHex(h: string): string { return (h.charAt(0) == "#") ? h.substring(1, 7) : h }
