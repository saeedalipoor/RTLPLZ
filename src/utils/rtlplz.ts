...
export default function RTLPLZ(input: string, node?: TextNode, reshape?: boolean, wrap?: boolean) {
  ...
  if (node && wrap && node.textAutoResize !== 'WIDTH_AND_HEIGHT') {
    wrapped = wrapLines(characters, node);
  }
  for (const [index, line] of (wrapped || characters).split('\n').entries()) {
    result.push(processLine(line))
  }

  const shouldReshape = reshape && ((!!node && hasDifferenttextStyles(node)) || is.ltr(result[0].split('').filter(c => !is.neutral(c) && !is.bracket(c))[0]))
